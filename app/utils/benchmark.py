...
class Benchmark:
    ...
    def start(self, benchmark_point="BenchmarkStartPoint"):
        ...
        return f"{benchmark_point} started at {self.start_timestamps[benchmark_point]}"
    ...
    def progress(self, benchmark_point):
        if benchmark_point in self.start_times:
            ...
            return f"{benchmark_point} progress in {elapsed_time:.6f} seconds at {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    ...
    def end(self, benchmark_point):
        if benchmark_point in self.start_times:
            ...
            return f"{benchmark_point} finished at {self.end_timestamps[benchmark_point]} after {elapsed_time:.6f} seconds"
        return f"Benchmark point '{benchmark_point}' not found. Start it first."
